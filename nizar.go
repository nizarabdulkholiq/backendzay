...
func InsertUserTagihan(db string, usertagihan UserTagihan) (insertedID interface{}) {
	insertResult, err := MongoConnect(db).Collection("usertagihan").InsertOne(context.TODO(),usertagihan)
	if err != nil {
		fmt.Printf("InsertUserTagihan: %v\n", err)
	}
	return insertResult.InsertedID
}
